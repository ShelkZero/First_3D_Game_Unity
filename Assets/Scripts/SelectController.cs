...
public class SelectController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (_cubeSelection != null)
        {
            ...
            {
                float xScalse = (_hit.point.x - hitDrag.point.x) * -1;
                float zScalse = _hit.point.z - hitDrag.point.z;

                if (xScalse < 0.0f && zScalse < 0.0f)
                    _cubeSelection.transform.localRotation = Quaternion.Euler(new Vector3(0, 180, 0));
                else if (xScalse < 0.0f)
                    _cubeSelection.transform.localRotation = Quaternion.Euler(new Vector3(0, 0, 180));
                else if (zScalse < 0.0f)
                ...
                _cubeSelection.transform.localScale = new Vector3(Mathf.Abs(xScalse), 1, Mathf.Abs(zScalse));
